...
import chat.tox.antox.utils.{ConstantsJ, NotificationOffsets}
...
class MissedCallNotification(context: Context, contact: ContactInfo, missedCallMessages: Seq[Message]) {
  ...
  val startCallPendingIntent = createChatPendingIntent(context, ConstantsJ.START_CALL, classOf[ChatActivityJ], contact.key)
  val replyPendingIntent = createChatPendingIntent(context, ConstantsJ.SWITCH_TO_FRIEND, classOf[ChatActivityJ], contact.key)
